...
class ManagerBasedRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: ManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Joint tracking data for plotting
        self.joint_tracking_data = {
            'target_m3': [],
            'actual_m3': [],
            'target_m4': [],
            'actual_m4': [],
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        goal = tensor = torch.tensor([3.8, 1.4], device=self.device) + action[0, [0, 1]]

        # Store initial data for this step
        timestamp = self.common_step_counter * self.step_dt
        ...
        # After decimation loop, store final joint positions
        current_joint_pos = self.scene['robot'].data.joint_pos[0][2:4]
        joint_error = torch.abs(goal - current_joint_pos)

        # Store tracking data
        self.joint_tracking_data['target_m3'].append(goal[0].cpu().item())
        self.joint_tracking_data['actual_m3'].append(current_joint_pos[0].cpu().item())
        self.joint_tracking_data['target_m4'].append(goal[1].cpu().item())
        self.joint_tracking_data['actual_m4'].append(current_joint_pos[1].cpu().item())
        self.joint_tracking_data['timestamps'].append(timestamp)
        self.joint_tracking_data['errors'].append(joint_error.cpu().numpy())

        # Save plot periodically or at end of episode
        # (will be called in reset or manually)
        ...
        if len(reset_env_ids) > 0:
            # Save joint tracking plot before reset
            if len(self.joint_tracking_data['timestamps']) > 0:
                self._save_joint_plot(filename=f'joint_tracking_episode_{self.common_step_counter}.png')
                # Clear tracking data for next episode
                self.joint_tracking_data = {
                    'target_m3': [],
                    'actual_m3': [],
                    'target_m4': [],
    ...
    def _save_joint_plot(self, filename='joint_tracking_test.png'):
        """Save the joint tracking plot to an image file."""
        if len(self.joint_tracking_data['timestamps']) == 0:
            return

        # Create new figure
        fig, axes = plt.subplots(3, 1, figsize=(12, 8))
        fig.suptitle('Joint Tracking: Target vs Actual', fontsize=14, fontweight='bold')

        # Plot M3 joint
        axes[0].plot(self.joint_tracking_data['timestamps'],
                     self.joint_tracking_data['target_m3'],
                     'r--', label='Target M3', linewidth=2)
        axes[0].plot(self.joint_tracking_data['timestamps'],
                     self.joint_tracking_data['actual_m3'],
                     'b-', label='Actual M3', linewidth=1.5)
        axes[0].set_ylabel('M3 Joint Position (rad)')
        axes[0].set_title('Joint M3 Tracking')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)

        # Plot M4 joint
        axes[1].plot(self.joint_tracking_data['timestamps'],
                     self.joint_tracking_data['target_m4'],
                     'r--', label='Target M4', linewidth=2)
        axes[1].plot(self.joint_tracking_data['timestamps'],
                     self.joint_tracking_data['actual_m4'],
                     'b-', label='Actual M4', linewidth=1.5)
        axes[1].set_ylabel('M4 Joint Position (rad)')
        axes[1].set_title('Joint M4 Tracking')
        axes[1].legend()
        axes[1].grid(True, alpha=0.3)

        # Plot joint errors
        errors = np.array(self.joint_tracking_data['errors'])
        if len(errors) > 0:
            axes[2].plot(self.joint_tracking_data['timestamps'], errors[:, 0], 'r-', label='M3 Error', linewidth=2)
            axes[2].plot(self.joint_tracking_data['timestamps'], errors[:, 1], 'b-', label='M4 Error', linewidth=2)
            axes[2].set_xlabel('Time (s)')
            axes[2].set_ylabel('Absolute Error (rad)')
            axes[2].set_title('Joint Position Error')
            axes[2].legend()
        ...
        plt.close(fig)
        print(f"[INFO] Joint tracking plot saved to {filename}")

    def close(self):
        # Save final joint tracking plot before closing
        if len(self.joint_tracking_data['timestamps']) > 0:
            self._save_joint_plot(filename='joint_tracking_final.png')
