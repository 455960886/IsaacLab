...
class image_features(ManagerTermBase):
    ...
    def depth_to_pointcloud_batch_gpu(self, depth_batch, fx, fy, cx, cy, num_points=1024, 
        ...
        # translation = torch.tensor([0.1654, 0.0, 0.0494 - 0.003], device=device)
        ...
        rand_thresh = np.random.uniform(-0.009, -0.006)
    ...
    def _apply_domain_randomization(
        ...
        kernel_size = int(torch.randint(1, 6, (1,)).item())
        if kernel_size % 2 == 0:
        ...
        noise_std = torch.rand(1).item() * 0.04 + 0.01  # Random std between 0-0.03
    ...
    def __call__(
        ...
        batch_points_tensor = self.depth_to_pointcloud_batch_gpu(
            ...
            cy,
            num_points=1024,
            save_ply_debug=False,  # Use same unified flag
            env_id=0,
