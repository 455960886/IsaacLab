...
class KeyboardController:
    ...
    def __init__(self, num_arm_joints=3, num_gripper_joints=1, arm_offsets=None, arm_scales=None):
        self.num_arm_joints = num_arm_joints
        self.num_gripper_joints = num_gripper_joints
        # 每次按键对归一化目标的增量。
        # 当前环境动作是 [-1, 1]，这里建议先用 0.05，手感会比较细。
        self.action_step = 0.05

        # 持续保存的机械臂目标动作（归一化到 [-1, 1]）。
        # 和之前“按住时给瞬时动作、松手就清零”不同，这里是保持式目标。
        self.arm_target = np.zeros(num_arm_joints, dtype=np.float32)

        self.gripper_open = False
        self._should_exit = False
        self._should_reset = False

        # 用于在 reset 后，把当前真实关节角反算回归一化动作目标，避免一开始就跳到中点。
        self.arm_offsets = np.zeros(num_arm_joints, dtype=np.float32) if arm_offsets is None else np.asarray(arm_offsets, dtype=np.float32)
        self.arm_scales = np.ones(num_arm_joints, dtype=np.float32) if arm_scales is None else np.asarray(arm_scales, dtype=np.float32)
        if self.arm_offsets.shape != (num_arm_joints,) or self.arm_scales.shape != (num_arm_joints,):
            raise ValueError(
                f"arm_offsets/arm_scales 形状必须是 ({num_arm_joints},)，当前分别为 {self.arm_offsets.shape} / {self.arm_scales.shape}"
            )
        ...
        print("  UP/DOWN: Adjust Joint M3 target (holds after release)")
        print("  F/V: Adjust Joint M4 target (holds after release)")
        print("  Q/E: Adjust Joint M5 target (holds after release)")
        print("\nGripper Controls:")
    ...
    def _on_keyboard_event(self, event, *args, **kwargs):
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            ...
            # Arm joints - press
                ...
                # 改成“保持式目标控制”：
                # 每次按键都在当前目标上做增减，并保持该目标，不在松手时清零。
                joint_idx, direction = self._key_mapping[event.input.name]
                self.arm_target[joint_idx] += direction * self.action_step
                self.arm_target = np.clip(self.arm_target, -1.0, 1.0)

        elif event.type == carb.input.KeyboardEventType.KEY_RELEASE:
            pass
    
    def sync_arm_targets_from_joint_pos(self, joint_pos_rad):
        """把当前真实关节角反算成归一化动作目标，并写回控制器内部状态。"""
        joint_pos_rad = np.asarray(joint_pos_rad, dtype=np.float32)
        if joint_pos_rad.shape != (self.num_arm_joints,):
            raise ValueError(
                f"joint_pos_rad 形状必须是 ({self.num_arm_joints},)，当前为 {joint_pos_rad.shape}"
            )

        normalized = (joint_pos_rad - self.arm_offsets) / self.arm_scales
        self.arm_target = np.clip(normalized, -1.0, 1.0).astype(np.float32)
    ...
    def get_action_tensor(self, num_envs, device):
        ...
        arm_tensor = torch.tensor(self.arm_target, dtype=torch.float32, device=device).repeat(num_envs, 1)
...
def main():
    ...
    controlled_joint_names = ["M3", "M4", "M5"]
    controlled_joint_ids = [joint_names_all.index(name) for name in controlled_joint_names]

    arm_action_cfg = env_cfg.actions.arm_action
    if not isinstance(arm_action_cfg.scale, dict) or not isinstance(arm_action_cfg.offset, dict):
        raise TypeError("当前手动控制脚本要求 arm_action 的 scale/offset 使用 dict 形式配置。")

    arm_offsets = np.array(
        [float(arm_action_cfg.offset[name]) for name in controlled_joint_names],
        dtype=np.float32,
    )
    arm_scales = np.array(
        [float(arm_action_cfg.scale[name]) for name in controlled_joint_names],
        dtype=np.float32,
    )
    if np.any(np.isclose(arm_scales, 0.0)):
        raise ValueError(f"arm_scales 中存在 0，无法进行归一化反算：{arm_scales}")
    ...
    # 创建键盘控制器。
    # 当前环境动作空间是：M3/M4/M5 三个机械臂关节 + 1 个夹爪二值动作，共 4 维。
    # 这里给控制器传入动作映射的 offset/scale，用于在 reset 后把当前真实关节角同步回归一化目标。
    ...
    # 启动时先和当前机器人实际姿态同步，避免默认目标与当前关节角不一致。
    current_m345 = robot.data.joint_pos[0, controlled_joint_ids].detach().cpu().numpy()
    controller.sync_arm_targets_from_joint_pos(current_m345)
    ...
    try:
        while simulation_app.is_running():
            if controller.should_reset():
                ...
                total_reward = 0.0
                step_count = 0
                # reset 后重新同步保持式目标，避免 reset 之后仍沿用旧 episode 的手动目标。
                current_m345 = robot.data.joint_pos[0, controlled_joint_ids].detach().cpu().numpy()
                controller.sync_arm_targets_from_joint_pos(current_m345)
            ...
            if terminated.any() or truncated.any():
                ...
                # episode 自动结束后同样要重新同步目标，避免控制器内部状态和机器人当前姿态脱节。
                current_m345 = robot.data.joint_pos[0, controlled_joint_ids].detach().cpu().numpy()
                controller.sync_arm_targets_from_joint_pos(current_m345)
