...
class image_features(ManagerTermBase):
    ...
    def _prepare_resnet_model(self, model_name: str, model_device: str) -> dict:
        """Prepare the ResNet model for inference with spatial softmax.
        
        Extracts layer3 features and applies spatial softmax to preserve spatial information
        while reducing dimensionality.
        Output shape: [B, 512] (256 channels × 2 coordinates)
        """
        ...
        def _load_model() -> torch.nn.Module:
            """Load the ResNet model up to layer3."""
            # Map model name to weights
            resnet_weights = {
            ...
            # Load full model
            full_model = getattr(models, model_name)(weights=resnet_weights[model_name]).eval()
        ...
        # def _inference(model, images: torch.Tensor) -> torch.Tensor:
        #     """Inference the ResNet model with averaged heatmap.
            
        #     Args:
        #         model: The ResNet model (up to layer3).
        #         images: The preprocessed image tensor. Shape is (num_envs, H, W, channel).
            
        #     Returns:
        #         The averaged heatmap features.
        #         Shape: [num_envs, 196] (14x14 flattened) for ResNet18/34
        #     """
        #     # Move image to model device and preprocess
        #     image_proc = images.to(model_device)
        #     image_proc = image_proc.permute(0, 3, 1, 2).float() / 255.0
        ...
        #     # Resize to 224x224 for consistent 14x14 feature maps
        #     image_proc = torch.nn.functional.interpolate(
        #         image_proc, 
        #         size=(224, 224), 
        #         mode='bilinear', 
        #         align_corners=False
        ...
        #     with torch.no_grad():
        #         # Extract layer3 features
        #         # ResNet18/34: [B, 256, 14, 14]
        #         # ResNet50/101: [B, 1024, 14, 14]
        #         feats = model(image_proc)

        #         # # Visualize features with input image
        #         # if self._viz_counter % 10 == 0:
        #         #     import os
        #         #     import cv2
        #         #     import numpy as np
                    
        #         #     save_dir = "resnet_features_viz"
        #         #     os.makedirs(save_dir, exist_ok=True)
                    
        #         #     B, C, H, W = feats.shape
        #         #     print(f"Input image shape: {images.shape}")
        #         #     print(f"Feature shape: [B={B}, C={C}, H={H}, W={W}]")
                    
        #         #     # Save original input image
        #         #     orig_img = images[0].cpu().numpy()  # [H, W, C]
        #         #     print(f"Original image shape for saving: {orig_img.shape}")
        #         #     orig_img = np.clip(orig_img, 0, 255).astype(np.uint8)
        #         #     orig_img_bgr = cv2.cvtColor(orig_img, cv2.COLOR_RGB2BGR)
                    
        #         #     # Generate averaged heatmap across all channels
        #         #     avg_heatmap = feats[0].mean(dim=0).cpu().numpy()  # [14, 14]
        #         #     print(f"Averaged heatmap shape: {avg_heatmap.shape}")
                    
        #         #     # Normalize
        #         #     avg_heatmap_norm = (avg_heatmap - avg_heatmap.min()) / (avg_heatmap.max() - avg_heatmap.min() + 1e-8)
        #         #     avg_heatmap_uint8 = (avg_heatmap_norm * 255).astype(np.uint8)
                    
        #         #     # Apply colormap and upscale to 224x224 for visualization
        #         #     heatmap_colored = cv2.applyColorMap(avg_heatmap_uint8, cv2.COLORMAP_JET)
        #         #     heatmap_upscaled = cv2.resize(heatmap_colored, (224, 224), interpolation=cv2.INTER_NEAREST)
                    
        #         #     # Resize original image to 224x224 for comparison
        #         #     orig_img_resized = cv2.resize(orig_img_bgr, (224, 224))
                    
        #         #     # Create side-by-side comparison: [image | heatmap]
        #         #     combined = np.hstack([orig_img_resized, heatmap_upscaled])
                    
        #         #     # Add text labels
        #         #     cv2.putText(combined, f"Input Image", (10, 30), 
        #         #                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        #         #     cv2.putText(combined, f"Avg Heatmap ({H}x{W})", (234, 30), 
        #         #                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    
        #         #     # Save combined image
        #         #     save_path = os.path.join(save_dir, f"averaged_heatmap_{self._viz_counter:06d}.png")
        #         #     cv2.imwrite(save_path, combined)
        #         #     print(f"Saved averaged heatmap visualization: {save_path}")
                
        #         # self._viz_counter += 1
                
        #         # Create averaged heatmap by averaging across all channels
        #         # [B, C, H, W] -> [B, H, W]
        #         avg_heatmap = feats.mean(dim=1)
                
        #         # Flatten spatial dimensions to create feature vector
        #         # [B, H, W] -> [B, H*W] = [B, 196] for 14x14
        #         features = avg_heatmap.flatten(start_dim=1)
        ...
        def _inference(model, images: torch.Tensor) -> torch.Tensor:
            ...
            # Resize for consistent feature map size
            image_proc = F.interpolate(image_proc, size=(224, 224), mode='bilinear', align_corners=False)
            
            with torch.no_grad():
                feats = model(image_proc)  # [B, 256, 14, 14]
                
                # Global Average Pooling
                features = feats.mean(dim=[2, 3])  # [B, 256]
