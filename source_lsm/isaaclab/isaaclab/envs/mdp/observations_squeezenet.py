...
class _SimplePerf:
    def __init__(self, enabled: bool, print_every: int = 200):
        self.enabled = enabled
        self.print_every = print_every
        self.sum_ms = defaultdict(float)
        self.cnt = defaultdict(int)
        self.it = 0

    def _time_block(self, key: str):
        """context manager like: with self.perf.block("xxx"): ..."""
        perf = self
        ...
        class _Ctx:
            def __enter__(self_inner):
                if not perf.enabled:
                    return None
                perf.it += 1
                self_inner.is_cuda = torch.cuda.is_available()
                if self_inner.is_cuda:
                    torch.cuda.synchronize()
                    self_inner.start = torch.cuda.Event(enable_timing=True)
                    self_inner.end = torch.cuda.Event(enable_timing=True)
                    self_inner.start.record()
                else:
            ...
            def __exit__(self_inner, exc_type, exc, tb):
                if not perf.enabled:
                    return False
                if self_inner.is_cuda:
                    self_inner.end.record()
                    torch.cuda.synchronize()
                    ms = self_inner.start.elapsed_time(self_inner.end)
                else:
                    ms = (time.perf_counter() - self_inner.t0) * 1000.0
                perf.sum_ms[key] += ms
                perf.cnt[key] += 1

                # 每 print_every 次打印一次平均耗时
                if perf.it % perf.print_every == 0:
                    parts = []
                    total = 0.0
                    # 按 key 排一下，方便看
                    for k in sorted(perf.sum_ms.keys()):
                        avg = perf.sum_ms[k] / max(1, perf.cnt[k])
                        parts.append(f"{k}: {avg:.3f} ms")
                        total += avg
                    print(f"[OBS_PERF] avg over ~{perf.print_every} calls | total≈{total:.3f} ms | " + " | ".join(parts))
                return False

        return _Ctx()

    def block(self, key: str):
        return self._time_block(key)
...
def image(
    ...
    # cv2.imwrite(f"/home/robo/下载/{step}.png", images)
    # import pdb
    # pdb.set_trace()
...
class image_features(ManagerTermBase):
    ...
    def __init__(self, cfg: ObservationTermCfg, env: ManagerBasedEnv):
        ...
        self._perf = _SimplePerf(
            enabled=bool(int(os.getenv("OBS_PROFILE", "0"))),
            print_every=int(os.getenv("OBS_PROFILE_EVERY", "200"))
        )
    ...
    def __call__(
        ...
        # --- 1) RGB取数（可能包含渲染/同步/搬运）---
        with self._perf.block("cam_rgb_fetch"):
            images = sensor.data.output[data_type]

        # --- 2) RGB裁剪 + 域随机化（blur/noise 等）---
        with self._perf.block("rgb_crop_dr"):
            images = images[:, 120:, :, :]
        ...
        # --- 3) ResNet前向 ---
        with self._perf.block("resnet_fwd"):
            features = self._inference_fn(self._model, images, **(inference_kwargs or {}))

        # --- 4) Depth取数 + 相机内参 ---
        with self._perf.block("cam_depth_fetch"):
            depth = env.scene.sensors[depth_cfg.name].data.output["distance_to_image_plane"]

        with self._perf.block("cam_intrinsics"):
        ...
        # --- 5) depth -> pointcloud（通常是大头之一）---
        with self._perf.block("depth_to_pcd"):
        ...
        # cache（这几行一般不贵，但你也可以计一下）
        with self._perf.block("pcd_cache_store"):
        ...
        # --- 6) PointNet2前向（通常是大头之一）---
        with self._perf.block("pointnet_fwd"):
            # 先别改你原来的 no_grad：profiling 阶段不影响
            with torch.no_grad():
                depth_features_batch = self._point_encoder(pts_input)

        # --- 7) 拼接/归一化/拷贝 ---
        with self._perf.block("feat_post"):
            ...
            features = features.detach().to(image_device)
