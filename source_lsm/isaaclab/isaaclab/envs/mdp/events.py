...
def randomize_object_pool_selection(
    ...
    """Balanced selection: shuffle env_ids, then assign object indices in round-robin (0,1,2,0,1,2...)."""

    # DEBUG: 记录是否 startup 调用（env_ids 为 None 时）
    is_startup_call = env_ids is None
    ...
    # Pre-alloc constants on device
    pos_active = torch.tensor([0.3254, 0.0, 0.0], device=env.device)
    pos_inactive = torch.tensor([100.0, 100.0, -10.0], device=env.device)
    quat_unit = torch.tensor([1.0, 0.0, 0.0, 0.0], device=env.device)

    # Shuffle envs, then round-robin assign (balanced coverage)
    perm = env_ids[torch.randperm(env_ids.numel(), device=env.device)]

    for i, env_idx_t in enumerate(perm):
        env_idx = int(env_idx_t.item())
        active_idx = int(i % num_objects)
        env.active_object_indices[env_idx] = active_idx

        for obj_idx in range(num_objects):
            pos_local = pos_active if obj_idx == active_idx else pos_inactive

            root_state = object_collection.data.default_object_state[env_idx, obj_idx].clone()
            root_state[:3] = pos_local + env.scene.env_origins[env_idx]
            root_state[3:7] = quat_unit
            root_state[7:] = 0.0
            ...
            object_collection.write_object_state_to_sim(
                ...
                object_ids=torch.tensor([obj_idx], device=env.device, dtype=torch.long),
            )

    # DEBUG: 只在 startup 打印一次统计，展示每个物体被分配为 active 的次数
    if is_startup_call:
        assigned = env.active_object_indices[env_ids].detach().cpu()  # (num_envs_selected,)
        counts = torch.bincount(assigned, minlength=num_objects).tolist()
        names = list(object_collection.object_names)
        msg = ", ".join([f"{n}={c}" for n, c in zip(names, counts)])
        print(f"[ObjectPool][BalancedStartup] num_envs={env_ids.numel()} -> {msg}")
