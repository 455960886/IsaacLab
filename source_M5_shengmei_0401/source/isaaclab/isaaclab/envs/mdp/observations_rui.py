...
from .depth_image_processing import DepthGradientSmoother
...
class image_features(ManagerTermBase):
    ...
    def __call__(
        ...
        # os.makedirs("/home/roborock/depth_gradient_output/data", exist_ok=True)

        # # Convert tensor to numpy and take first image in batch
        # depth_np = depth[0].cpu().numpy().squeeze()  # squeeze to remove channel dimension if present
        ...
        # # Save
        # cv2.imwrite("/home/roborock/depth_gradient_output/data/depth.png", depth_vis)

        # if not hasattr(self, '_depth_smoother'):
        #     self._depth_smoother = DepthGradientSmoother(
        #         threshold_percentile=50,  # Adjust to control which pixels
        #         kernel_size=9,           # Large = strong smoothing
        #         sigma=3.0                 # Large = strong smoothing
        #     )

        # depth = self._depth_smoother.smooth(depth)
    ...
    def _prepare_resnet_model(self, model_name: str, model_device: str) -> dict:
        """Prepare the ResNet model for inference with channel slicing + 1x1 conv projection.
        
        Extracts layer3 features and applies two-stage compression:
        1. Select first 128 channels from 256 (channel slicing, no params)
        2. 128 → 16 channels via frozen 1x1 conv + ReLU
        The spatial dimensions are preserved at 14×14.
        Output shape: [B, 3136] (16 channels × 14 × 14 flattened)
        """
        ...
        def _load_model() -> torch.nn.Module:
            """Load the ResNet model up to layer3 with channel slicing + 1x1 conv projection."""
            ...
            # Compression:
            # Stage 1: Slice first 128 channels (no learnable params)
            # Stage 2: 128 → 16 channels via 1x1 conv (frozen)
            # Output: [B, 16, 14, 14]
            # IMPORTANT: Use fixed seed for reproducible initialization
            ...
            # fixed weight
            feature_projection.requires_grad_(False)
            # Combine backbone and projection
            model = nn.Sequential(backbone, feature_projection)
            return model.to(model_device)
        
        def _inference(model, images: torch.Tensor) -> torch.Tensor:
            """Inference the ResNet model with channel slicing + 1x1 conv projection.
            
            Args:
                model: The ResNet model (up to layer3) + projection layer.
                images: The preprocessed image tensor. Shape is (num_envs, H, W, channel).
            
            Returns:
                The projected features.
                Shape: [num_envs, 3136] (16 channels × 14 × 14) for ResNet18/34
            """
            ...
            with torch.no_grad():
                # Extract layer3 features from backbone
                # ResNet18/34 layer3: [B, 256, 14, 14]
                backbone_feats = model[0](image_proc)  # model[0] is the backbone
                
                # Stage 1: Slice first 128 channels (no learnable params)
                # [B, 256, 14, 14] -> [B, 128, 14, 14]
                sliced_feats = backbone_feats[:, :128, :, :]
                
                # Stage 2: Apply 1x1 conv projection (128 → 16)
                # [B, 128, 14, 14] -> [B, 16, 14, 14]
                feats = model[1](sliced_feats)  # model[1] is the feature_projection

                # Flatten spatial and channel dimensions to create feature vector
                # [B, 16, 14, 14] -> [B, 16*14*14] = [B, 3136]
